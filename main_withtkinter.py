...
def forward_ik_tk(robot,q1,q2,q3,q4,q5,q6):
    '''Fonction qui demande à l'utilisateur de rentrer un angle pour chaque membre et qui deplace le robot vers cette position'''
    #test value for theta
    theta = np.array([q1, q2, q3, q4, q5, q6])
    robot.forward(theta)
...
def animate(robot):
    frames = [F.from_euler_3(np.array([0.5 * pi, 0., pi]), np.array([[0.28127], [0.], [1.13182]])),
              F.from_euler_3(np.array([0.25 * pi, 0., 0.75 * pi]), np.array([[0.48127], [0.], [1.13182]])),
              F.from_euler_3(np.array([0.5 * pi, 0., pi]), np.array([[0.48127], [0.], [0.63182]]))]
    time_points = np.array([0., 6., 10.])
    trajectory = RobotTrajectory(robot, frames, time_points)
    trajectory.show(motion="p2p")

##Terminal dans tkinter
class RedirectedOutput:
    ...
    def flush(self):
        pass  # Needed for compatibility with sys.stdout and stderr

def run():
    for i in range(10):
        print(f"Line {i+1}: This is a sample terminal output.")
...
#Interface
    ...
    # Create a ScrolledText widget to display output
    text_area = ScrolledText(fenetre, wrap="word", width=80, height=10)
    text_area.grid(row=1, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")

    # Redirect sys.stdout and sys.stderr custom class
    sys.stdout = RedirectedOutput(text_area)
    sys.stderr = RedirectedOutput(text_area)
    # Create a frame for the canvas to ensure proper placement
    canvas = FigureCanvasTkAgg(robot.figure, master=fenetre)
    canvas.draw()
    canvas.get_tk_widget().grid(row=0, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")

    robot.canvas = canvas  # Assign the canvas to robot object

    # Label for options
    label = Label(fenetre, text="Choose option")
    label.grid(row=2, column=0, columnspan=2, pady=5)

    # Buttons layout
    bouton_1 = Button(fenetre, text="Photo", command=lambda: show_image("robot.png"))
    bouton_1.grid(row=3, column=0, padx=5, pady=5)

    bouton_2 = Button(fenetre, text="Go to home position", command=lambda: go_home(robot))
    bouton_2.grid(row=3, column=1, padx=5, pady=5)

    bouton_3 = Button(fenetre, text="Parametres de denavit-hartenberg", command=lambda: print(dh_params))
    bouton_3.grid(row=4, column=0, padx=5, pady=5)

    bouton_4 = Button(fenetre, text="Forward Kinematics", command=lambda: forward_ik_tk(robot,float(entry_var1.get()),float(entry_var2.get()),float(entry_var3.get()),float(entry_var4.get()),float(entry_var5.get()),float(entry_var6.get())))
    bouton_4.grid(row=4, column=1, padx=5, pady=5)

    bouton_5 = Button(fenetre, text="Inverse Kinematics", command=lambda: inverse_ik(robot))
    bouton_5.grid(row=5, column=0, padx=5, pady=5)

    bouton_6 = Button(fenetre, text="Animation", command=lambda: animate(robot))
    bouton_6.grid(row=5, column=1, padx=5, pady=5)

    bouton_7 = Button(fenetre, text="Quit", command=fenetre.quit)
    bouton_7.grid(row=6, column=0, columnspan=2, pady=10)

    var1 = var2 = var3 = var4 = var5 = var6 = 0

    # Create labels for the variables
    label_var1 = Label(fenetre, text=f"Q 1:")
    label_var1.grid(row=3, column=2, padx=5, pady=5)

    label_var2 = Label(fenetre, text=f"Q 2:")
    label_var2.grid(row=3, column=4, padx=5, pady=5)

    label_var3 = Label(fenetre, text=f"Q 3:")
    label_var3.grid(row=4, column=2, padx=5, pady=5)

    label_var4 = Label(fenetre, text=f"Q 4:")
    label_var4.grid(row=4, column=4, padx=5, pady=5)

    label_var5 = Label(fenetre, text=f"Q 5:")
    label_var5.grid(row=5, column=2, padx=5, pady=5)

    label_var6 = Label(fenetre, text=f"Q 6:")
    label_var6.grid(row=5, column=4, padx=5, pady=5)

    # Create entry widgets for each variable
    entry_var1 = Entry(fenetre, width=10)
    entry_var1.grid(row=3, column=3, padx=5, pady=5)

    entry_var2 = Entry(fenetre,  width=10)
    entry_var2.grid(row=3, column=5, padx=5, pady=5)

    entry_var3 = Entry(fenetre,  width=10)
    entry_var3.grid(row=4, column=3, padx=5, pady=5)

    entry_var4 = Entry(fenetre,  width=10)
    entry_var4.grid(row=4, column=5, padx=5, pady=5)

    entry_var5 = Entry(fenetre,  width=10)
    entry_var5.grid(row=5, column=3, padx=5, pady=5)

    entry_var6 = Entry(fenetre,  width=10)
    entry_var6.grid(row=5, column=5, padx=5, pady=5)
